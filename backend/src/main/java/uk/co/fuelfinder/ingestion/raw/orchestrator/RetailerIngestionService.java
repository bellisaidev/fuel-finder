...
import uk.co.fuelfinder.ingestion.raw.client.FuelFinderFeedClient;
import uk.co.fuelfinder.ingestion.raw.client.FuelFinderFeedClient.FetchResult;
import uk.co.fuelfinder.ingestion.normalize.NormalizedRecord;
import uk.co.fuelfinder.ingestion.raw.mapper.RetailerFeedMapper;
import uk.co.fuelfinder.ingestion.raw.writer.LatestPriceJdbcRepository;
import uk.co.fuelfinder.ingestion.raw.writer.PriceObservationJdbcRepository;
...
public class RetailerIngestionService {
    ...
    private final FuelFinderFeedClient client;
    ...
    private final PriceObservationJdbcRepository priceObservationJdbcRepository;
    private final LatestPriceJdbcRepository latestPriceJdbcRepository;

    private final GeometryFactory geometryFactory = new GeometryFactory(new PrecisionModel(), 4326);

    public RetailerIngestionService(FuelFinderFeedClient client,
                                    RetailerFeedMapper mapper,
                                    ObjectMapper objectMapper,
                                    RawFeedFetchRepository rawFeedFetchRepository,
                                    StationRepository stationRepository,
                                    PriceObservationJdbcRepository priceObservationJdbcRepository,
                                    LatestPriceJdbcRepository latestPriceJdbcRepository) {
        this.client = client;
        this.mapper = mapper;
        this.objectMapper = objectMapper;
        this.rawFeedFetchRepository = rawFeedFetchRepository;
        this.stationRepository = stationRepository;
        this.priceObservationJdbcRepository = priceObservationJdbcRepository;
        this.latestPriceJdbcRepository = latestPriceJdbcRepository;
    }
    ...
    public IngestionSummary ingest(RetailerEntity retailer) {
        ...
        for (NormalizedRecord r : records) {
            ...
            int inserted = priceObservationJdbcRepository.insertIfNotExists(
                    obsId,
                    station.getId(),
                    r.fuelType(),
                    r.pricePence(),
            ...
            if (inserted == 1) {
                ...
                latestPriceJdbcRepository.upsert(
                        station.getId(),
                        r.fuelType(),
                        r.pricePence(),
                        observedAt.toInstant(),
